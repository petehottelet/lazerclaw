...
function WelcomePage({ onEnter, onEnterAsGuest, onShowAbout, introActive }) {
  ...
  useEffect(() => {
    ...
    let activeBolts = []
    let nextSpawn = 2500 + Math.random() * 4500
    ...
    function draw(now) {
      ...
      if (elapsed >= nextSpawn) {
        spawnBolt(W, H)
        elapsed = 0
        nextSpawn = 1500 + Math.random() * 4000
        if (Math.random() < 0.07) {
      ...
      if (activeBolts.length > 1) activeBolts.splice(0, activeBolts.length - 1)
  ...
  useEffect(() => {
    ...
    let laserBolts = []
    let nextLaser = 2000 + Math.random() * 2500
    ...
    function draw(now) {
      ...
      if (elapsed >= nextLaser) {
        spawnLaserBolt(W, H)
        elapsed = 0
        nextLaser = 1750 + Math.random() * 2250
      }
      ...
      if (laserBolts.length > 1) laserBolts.splice(0, laserBolts.length - 1)
...
class ErrorBoundary extends React.Component {
  ...
  render() {
    if (this.state.error) {
      return (
        <div style={{ padding: 40, color: '#f87171', background: '#111827', fontFamily: 'monospace', whiteSpace: 'pre-wrap' }}>
          <h2 style={{ color: '#ef4444' }}>LazerClaw crashed</h2>
          <p>{this.state.error.message}</p>
          <pre style={{ fontSize: 12, color: '#9ca3af', maxHeight: 400, overflow: 'auto' }}>{this.state.error.stack}</pre>
        </div>
      )
    }
