...
export default function CanvasArea({ canvasState }) {
  ...
  return (
    ...
    >
      ...
      <button
        className={`absolute bottom-3 left-3 z-10 flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-medium shadow-sm border transition-colors ${
          showTokenLabels
        ...
        onClick={() => { setShowTokenLabels(v => !v); showTokenLabelsRef.current = !showTokenLabelsRef.current; canvasRef.current?.requestRenderAll() }}
      ...
      >
        ...
        {showTokenLabels ? 'Labels on' : 'Labels'}
      </button>


      {ctxMenu && (
        <>
          <div className="fixed inset-0 z-40" onClick={() => setCtxMenu(null)} onMouseDown={(e) => e.stopPropagation()} />
          <div
            className={`fixed z-50 border rounded-lg shadow-xl py-1 min-w-[160px] text-xs ${
