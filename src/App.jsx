  ...
  if (isMobile) {
    return (
      <div className={`h-screen w-screen flex flex-col select-none overflow-hidden ${darkMode ? 'dark bg-gray-900' : 'bg-gray-100'}`}>
        ...
        <MobileBottomSheet
          isOpen={mobileSubSheet === 'canvas-size'}
          onClose={() => setMobileSubSheet(null)}
          title="Canvas Size"
          darkMode={darkMode}
          height="40vh"
          ...
          <div className="p-4 flex flex-col gap-3">
            <div className="flex gap-3">
              <label className="flex-1">
                <span className={`text-xs font-medium ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>Width (px)</span>
                <input type="number" value={canvasState.canvasW} min={10} max={10000}
                  onChange={e => { const v = parseInt(e.target.value); if (v >= 10 && v <= 10000) canvasState.setCanvasW(v) }}
                  className={`w-full mt-1 rounded px-3 py-2 text-sm border outline-none ${darkMode ? 'bg-gray-700 border-gray-600 text-white' : 'bg-white border-gray-300 text-gray-900'}`}
                />
              </label>
              <label className="flex-1">
                <span className={`text-xs font-medium ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>Height (px)</span>
                <input type="number" value={canvasState.canvasH} min={10} max={10000}
                  onChange={e => { const v = parseInt(e.target.value); if (v >= 10 && v <= 10000) canvasState.setCanvasH(v) }}
                  className={`w-full mt-1 rounded px-3 py-2 text-sm border outline-none ${darkMode ? 'bg-gray-700 border-gray-600 text-white' : 'bg-white border-gray-300 text-gray-900'}`}
                />
              </label>
            </div>
